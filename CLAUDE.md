...
- **The UI is a light theme, and flipping `.streamlit/config.toml`'s `[theme]` alone is not enough to change it.** Four elements in `front_end/app.py` carry their own hardcoded colours and ignore the Streamlit theme: the fixed `#proj-header` bar (CSS block near the top, plus its inline title/subtitle colours), `render_log()`'s console background and per-status text colours, the AIDA explanation card (container gradient, header strip, and the body/heading/error text colours near the end of the file), and the plotly chart/placeholder border. A dark theme was previously assumed in all of them (pale text on dark cards), so switching either direction means editing all of them together. New text colours were checked against WCAG AA (>=4.5:1) on their backgrounds. The Result 3D viewer's axis text follows the Streamlit theme automatically (`st.plotly_chart` default), and its legend text is deliberately hardcoded black on a white legend box, which is correct in both themes.
