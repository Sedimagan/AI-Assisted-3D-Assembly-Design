...
- **Viewer state machine: upload -> Original -> (prediction + reconstruction done) -> Result.** In `app.py` the right-hand 3D viewer (`col_right`) is drawn BEFORE the prediction code (`col_left`) in every script run, so anything left in session state or on disk from an earlier file shows up the moment a new file loads. `_reset_prediction_state()` (called on every new upload and by "Predict another" / "Reset All") clears inference, reconstruction, AIDA text and deletes `.logs/reconstructed.glb`. The viewer starts on "Original" (only the uploaded model), switches itself to "Result" once `inference_done_for` and `recon_done_for` both equal the current file (`viewer_auto_done_for` makes that happen once, so the user's later choice sticks), and draws the "Reconstructed" group only then and only in Result view, with `visible="legendonly"` (off until the legend entry or the "All rebuilt" button turns it on). A widget's value cannot be set after it is drawn, so resets go through the `_viewer_mode_next` pending flag applied just before the segmented control is created. Before this, "Predict another" left `recon_done_for` set, so re-uploading the same file skipped reconstruction and showed the stale rebuilt parts immediately.
