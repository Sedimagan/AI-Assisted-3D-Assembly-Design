...
runs = [
    ...
    dict(id="R21", date="26 Jun\n00:26", graphs="89 real",
         note="3 new categories · no skip/edge filters\n89 STEP → 89 graphs · 0 errors · 0 timeouts\nMachine design=28 · Mech.Eng=31 · Tools=30\n5-Fold CV · Mean AUC=0.428±0.174 · Mean AP=0.799±0.063",
         val_auc=0.731, test_auc=0.500, test_ap=0.834, synthetic=False),
    dict(id="R22", date="26 Jun\n07:34", graphs="239 real",
         note="3 categories · Best_models_for_training · 3× more data\n300 STEP → 239 graphs · 19 timeouts · ~8 errors\nTools=100 · Machine design=100 · Mech.Eng=100\n5-Fold CV · Mean AUC=0.588±0.050 · Mean AP=0.767±0.031",
         val_auc=0.625, test_auc=0.626, test_ap=0.772, synthetic=False),
    dict(id="R23", date="27 Jun\n02:06", graphs="156 real",
         note="Tools-only · Best_models_for_training · domain focus\n197 STEP → 156 graphs · 41 skipped/timeout\nSingle category test vs 3-cat R22\n5-Fold CV · Mean AUC=0.460±0.039 · Mean AP=0.682±0.024",
         val_auc=0.710, test_auc=0.406, test_ap=0.632, synthetic=False),
    dict(id="R24", date="27 Jun\n11:39", graphs="159 real",
         note="Machine_design-only · Best_models_for_training · domain focus\n232 STEP → 159 graphs · 73 skipped/timeout\nSingle category test vs 3-cat R22\n5-Fold CV · Mean AUC=0.559±0.080 · Mean AP=0.709±0.063",
         val_auc=0.652, test_auc=0.616, test_ap=0.748, synthetic=False),
    dict(id="R25", date="27 Jun\n20:53", graphs="1006 real",
         note="Mechanical Engineering · Best_models_for_training · 6.4× more data\n1212 STEP → 1006 graphs · 28 timeouts · 178 skipped\nSingle-category ME focus vs Tools-only R23\n5-Fold CV · Mean AUC=0.731±0.072 · Mean AP=0.863±0.031",
         val_auc=1.0000, test_auc=0.8286, test_ap=0.9216, synthetic=False),
    dict(id="R26", date="29 Jun\n14:17", graphs="707 real",
         note="ME Medium Nodes Medium Edges · Best_models_for_training\n793 STEP → 707 graphs · 32 quarantined · 11 timeouts\nMedium-complexity assemblies vs R25 low-node subset\n5-Fold CV · Mean AUC=0.416±0.049 · Mean AP=0.759±0.022",
         val_auc=0.7051, test_auc=0.4861, test_ap=0.7978, synthetic=False),
]
...
log_colors = {
    ...
    "R18": "#38bdf8", "R19": "#4f8ef7", "R20": "#e879f9", "R21": "#22d3ee",
    "R22": "#34d399",
    "R23": "#fb923c",
    "R24": "#a78bfa",
    "R25": "#f472b6",
...
table_data = [
    [r["id"], r["date"].replace("\n", " "),
     "22+6-dim · 707g · ME-MN-ME" if r["id"] == "R26" else "22+6-dim · 1006g · ME" if r["id"] == "R25" else "22+6-dim · 159g · Machine_design" if r["id"] == "R24" else "22+6-dim · 156g · Tools" if r["id"] == "R23" else "22+6-dim · 239g · 3cat" if r["id"] == "R22" else "22+6-dim · 89g · 3cat" if r["id"] == "R21" else "22+6-dim · 444g · 38cat" if r["id"] == "R20" else "22+6-dim · 995g" if r["id"] in ("R18","R19") else "13-dim" if r["id"] in ("R1","R3","R6") else "21-dim · 1760g" if r["id"] == "R17" else "21-dim · bbox+affine" if r["id"] == "R16" else "18-dim · P1-P6" if r["id"] == "R15" else "16-dim (5-fold CV)" if r["id"] in ("R12","R13","R14") else "16-dim",
     r["graphs"],
...
for (row, col), cell in tbl.get_celld().items():
    ...
    if row > 0 and runs[row-1]["id"] == "R21":
        cell.set_facecolor("#082a2e")
        cell.set_text_props(color="#22d3ee", fontweight="bold")
    if row > 0 and runs[row-1]["id"] == "R22":
        cell.set_facecolor("#062d1a")
        cell.set_text_props(color="#34d399", fontweight="bold")
    if row > 0 and runs[row-1]["id"] == "R23":
        cell.set_facecolor("#2d1800")
        cell.set_text_props(color="#fb923c", fontweight="bold")
    if row > 0 and runs[row-1]["id"] == "R24":
        cell.set_facecolor("#1a0b2e")
        cell.set_text_props(color="#a78bfa", fontweight="bold")
    if row > 0 and runs[row-1]["id"] == "R25":
        cell.set_facecolor("#2d0a1a")
        cell.set_text_props(color="#f472b6", fontweight="bold")
    if row > 0 and runs[row-1]["id"] == "R26":
        cell.set_facecolor("#0a2d12")
        cell.set_text_props(color="#86efac", fontweight="bold")
...
ax2.text(0.5, 0.99, "Change Log — R1 to R26", transform=ax2.transAxes,
